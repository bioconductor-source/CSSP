...
setMethod("cssp.power",
          ...
          definition=function(fit,x,ite=100,fold=1,min.count =10,useC=FALSE,qval=0.05)
          {
            ...
            for(i in seq_len(ite))              {
              ...
              pval.sim <- pnbinom(y.back,mu=fit@mu.chip*x/fit@lambday,size=fit@b,lower=F)
            ...
            if(useC==TRUE)              {
              ...
              for(i in seq_len(n.block+1))                {
                ...
                bin.pow <- bin.pow+ .Call("binpower_pval",
                                          ...
                                          min.count,
                                          fit@b,
                                          fit@mu.chip[id.block]*x/fit@lambday,a)
              }
              thr <- apply(cbind(fold*fit@mu.chip*x/fit@lambday,min.count),1,max)
              ptail.bind <- matrix(0,nrow=fit@n,ncol=fit@k)
              for(j in seq_len(fit@k)){
                ptail.bind[,j] <- pgamma(thr,shape=fit@post.shape.sig[,j],scale=fit@post.scale.sig[,j]*x/fit@lambday,lower=FALSE)
              }
              w.pow <- sum(as.vector(apply(fit@post.p.sig*ptail.bind,1,sum))*fit@post.p.bind)
              bin.pow <- bin.pow/w.pow
              
            }else{
              crit.val <- qnbinom(1-a,size=fit@b,mu=fit@mu.chip/fit@lambday*x)
              ...
              ptail.back <- rep(pgamma(thr,shape=fit@post.shape.back,scale=fit@post.scale.back*x/fit@lambday,lower=FALSE),ite)
              mean.tail.bind <- matrix(0,nrow=fit@n*ite,ncol=fit@k)
              for(j in seq_len(fit@k)){
                mean.tail.bind[,j] <- qgamma(ptail.bind[,j]*runif(fit@n*ite),shape=fit@post.shape.sig[,j],scale=fit@post.scale.sig[,j]*x/fit@lambday,lower=FALSE)
              }
